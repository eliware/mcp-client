...
function createTransport({ transport, url, token, headers, command, args, env, dispatcher, TransportClass, HTTPTransportClass = StreamableHTTPClientTransport, SSETransportClass = SSEClientTransport, StdioTransportClass = StdioClientTransport }) {
  if (TransportClass) {
  ...
  const options = { requestInit: { headers: resolveAuthHeaders(token, headers), ...(dispatcher ? { dispatcher } : {}) } };
...
export async function mcpClient(options) {
  const {
    ...
    forceIPv4 = process.platform === 'win32',
  ...
  const dispatcher = forceIPv4 && transport !== 'stdio' ? new Agent({ connect: { family: 4 } }) : undefined;
  ...
  const connect = async () => {
    ...
    activeTransport = createTransport({ transport, url, token: authToken, headers, command, args, env, dispatcher, TransportClass, HTTPTransportClass, SSETransportClass, StdioTransportClass });
